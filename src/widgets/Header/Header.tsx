...
export const Header = () => {
	...
	return (
		<UiHeader 
				logo={<Link href='/' onMouseEnter={mouseEnterHandle} onMouseLeave={mouseLeaveHandle} >
					<Image width={200} height={50}  src='/img/header/logo.svg' alt='' />
				</Link>
				}

				rightHeaderContent={(
					<Socials handleMouseEnter={mouseEnterHandle} handleMouseLeave={mouseLeaveHandle}/>
				)}
			...
			>
				<nav className='hidden xl:flex gap-x-12 font-semibold' onMouseEnter={mouseEnterHandle} onMouseLeave={mouseLeaveHandle}>
						{headerNavigationList.map(({ name, path }, idx) => (
							<Link key={idx} href={path} className='text-[#696c6d] hover:text-primary transition'>{name}</Link>
						))}
				</nav>
		</UiHeader>
	)
